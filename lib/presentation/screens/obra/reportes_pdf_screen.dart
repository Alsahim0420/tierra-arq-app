...
/// Pantalla para listar y mostrar los reportes PDF de una obra
class ReportesPdfScreen extends StatefulWidget {
  const ReportesPdfScreen({
    super.key,
    required this.obraId,
    required this.obraTitle,
  });

  final String obraId;
  final String obraTitle;
...
class _ReportesPdfScreenState extends State<ReportesPdfScreen> {
  ...
  Future<void> _loadReportes() async {
    ...
    try {
      print('📄 [ReportesPdfScreen] Cargando reportes para obra: ${widget.obraId}');
      final obraRepository = di.getIt<ObraRepository>();
      print('📄 [ReportesPdfScreen] Repository obtenido, llamando getReportesPdf...');
      final reportes = await obraRepository.getReportesPdf(
        obraId: widget.obraId,
        page: 1,
        limit: 100, // Obtener todos los reportes
      );

      print('📄 [ReportesPdfScreen] Reportes obtenidos: ${reportes.length}');
      setState(() {
        _reportes = reportes;
        _isLoading = false;
      });
    } catch (e, stackTrace) {
      print('❌ [ReportesPdfScreen] Error al cargar reportes: $e');
      print('❌ [ReportesPdfScreen] Stack trace: $stackTrace');
      setState(() {
  ...
  Future<void> _openPdf(ReportePdfEntity reporte) async {
    try {
      print('📄 [ReportesPdfScreen] Abriendo PDF: ${reporte.nombre}');
      print('📄 [ReportesPdfScreen] Reporte ID: ${reporte.id}');
      ...
      print('📄 [ReportesPdfScreen] Descargando PDF desde el backend...');
      final obraRepository = di.getIt<ObraRepository>();
      
      Uint8List pdfBytes;
      try {
        pdfBytes = await obraRepository.downloadReportePdf(reporte.id);
        print('📄 [ReportesPdfScreen] PDF descargado exitosamente: ${pdfBytes.length} bytes');
      } catch (e) {
        print('❌ [ReportesPdfScreen] Error al descargar PDF: $e');
        rethrow;
      }
      ...
      print('❌ [ReportesPdfScreen] Error al abrir PDF: $e');
      print('❌ [ReportesPdfScreen] Stack trace: $stackTrace');
