...
class _TareaDetailModalState extends State<TareaDetailModal> {
  ...
  bool _userModifiedState =
      false; // Flag para saber si el usuario modificó el estado manualmente
  String?
  _lastKnownTareaState; // Último estado conocido de la tarea desde el backend
  bool _isUserSelecting =
      false; // Flag para indicar que el usuario está seleccionando un valor
                      ...
                      children: [
                        ...
                        Container(
                          ...
                          child: Builder(
                            builder: (context) {
                              ...
                              final normalizedSelected = _normalizeState(
                                _selectedState,
                              );
                              String? validValue;
                              ...
                              validValue ??=
                                  'pendiente'; // Si no se encuentra, usar pendiente
                              ...
                              final itemsValues = _estados
                                  .map((e) => e)
                                  .toList();
                              final matchingItems = itemsValues
                                  .where((v) => v == validValue)
                                  .length;
                              if (matchingItems != 1) {}

                              return DropdownButtonFormField<String>(
                                key: ValueKey(
                                  'dropdown_$validValue',
                                ), // Key único para el dropdown
                                value: validValue,
                                ...
                                selectedItemBuilder: (BuildContext context) {
                                  return _estados.map((estado) {
                                    final stateColor =
                                        FormatUtils.getStateColor(estado);
                                    return Row(
                                      children: [
                                        Container(
                                          width: 10,
                                ...
                                items: _estados.map((estado) {
                                  final stateColor = FormatUtils.getStateColor(
                                    estado,
                                  );
                                  return DropdownMenuItem<String>(
                                    key: ValueKey(
                                      estado,
                                    ), // Agregar key único para evitar duplicados
                                    value: estado,
                                    child: Row(
                                      children: [
                                        Container(
                                ...
                                onChanged: (value) {
                                  if (value != null) {
                                    ...
                                    setState(() {
                                      ...
                                      _userModifiedState =
                                          true; // Marcar que el usuario modificó el estado
                                      _lastKnownTareaState =
                                          value; // Actualizar el último estado conocido para evitar que se sobrescriba
                                    });
                        ...
                        Row(
                          ...
                          children: [
                            ...
                            TextButton.icon(
                              onPressed: () =>
                                  _openObservationModal(context, tarea),
                              icon: Icon(
                        ...
                        SizedBox(
                          ...
                          child: Builder(
                            builder: (context) {
                              ...
                              final hasEvidenceChanges = _hasEvidenceChanges(
                                tarea,
                              );
                              ...
                              final hasNewEvidences =
                                  validPendingEvidences.length >
                                      tarea.evidences.length ||
                                  validPendingEvidences.any(
                                    (e) => !tarea.evidences.contains(e),
                                  );
    ...
    showModalBottomSheet(
      ...
      builder: (context) => BlocBuilder<ObraBloc, ObraState>(
        builder: (context, obraState) {
          ...
          return ObservationModal(tarea: updatedTarea, obraId: obraId);
        },
      ),
    ).then((saved) {
  ...
  Future<void> _saveChanges(BuildContext context) async {
    ...
    */
    ...
    if (widget.isAdmin &&
      ...
      if (stateChanged) {
        developer.log(
          '  [UI] Estado cambió manualmente',
          name: 'TareaStateFlow',
        );
        developer.log(
          '  [UI] Estado anterior: ${_normalizeState(tarea.state)}',
          name: 'TareaStateFlow',
        );
        developer.log(
          '  [UI] Estado nuevo: $_selectedState',
          name: 'TareaStateFlow',
        );
        ...
        developer.log(
          '  [UI] Disparando evento UpdateTareaState...',
          name: 'TareaStateFlow',
        );
        ...
        try {
          if (!mounted) return;
          developer.log(
            '  [UI] Esperando respuesta del BLoC...',
            name: 'TareaStateFlow',
          );
          ...
          developer.log(
            '  [UI] Respuesta recibida del BLoC',
            name: 'TareaStateFlow',
          );
          if (newState is TareaLoaded) {
            developer.log(
              '  [UI] Estado es TareaLoaded con ${newState.tareas.length} tareas',
              name: 'TareaStateFlow',
            );
            ...
            developer.log(
              '  [UI] Tarea actualizada - id: ${updatedTarea.id}',
              name: 'TareaStateFlow',
            );
            developer.log(
              '  [UI] Tarea actualizada - state: ${updatedTarea.state}',
              name: 'TareaStateFlow',
            );
          } else if (newState is TareaError) {
            developer.log(
              '  [UI] Error recibido del BLoC: ${newState.message}',
              name: 'TareaStateFlow',
            );
          }
        } catch (e) {
          developer.log(
            '  [UI] Error esperando respuesta del BLoC: $e',
            name: 'TareaStateFlow',
          );
          if (!mounted) return;
        }
        ...
        developer.log(
          '  [UI] Auto-actualización de estado por evidencias',
          name: 'TareaStateFlow',
        );
    ...
    if (mounted) {
      developer.log(
        '  [UI] Cerrando modal y actualizando estados de obras...',
        name: 'TareaStateFlow',
      );
